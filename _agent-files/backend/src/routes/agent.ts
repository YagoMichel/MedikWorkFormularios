...
//
// Endpoints expuestos al agente n8n. Cada endpoint corresponde a una "tool"
// que el AI Agent de n8n llama via HTTP.
// Auth: JWT de usuario con role AGENT o ADMIN.
//
// Las reglas de negocio (cupo, validacion de batch, etc.) viven aqui,
// no en n8n. n8n solo orquesta conversacion + tool calling.
...
router.get('/capacity', async (req, res) => {
  ...
  const citas = await prisma.appointment.findMany({
    where: { date: { gte: desde, lte: hasta }, status: { notIn: ['CANCELADA', 'NO_ASISTIO'] } },
    select: { date: true },
  });
  const ocupados = new Map<string, number>();
  for (const c of citas) {
    const key = c.date.toISOString().slice(0, 10);
    ocupados.set(key, (ocupados.get(key) || 0) + 1);
  }
  ...
  for (let d = new Date(desde); d <= hasta; d.setDate(d.getDate() + 1)) {
    ...
    const max = ov?.max ?? DEFAULT_CAP;
    const ocup = ocupados.get(key) || 0;
    dias.push({
      fecha: key,
      cupo_total: max,
      ocupado: ocup,
      disponible: max - ocup,
      alcanza: max - ocup >= required,
    });
  }
  res.json({
    pacientes_requeridos: required,
    dias,
    sugerencias: dias.filter((d) => d.alcanza).slice(0, 5).map((d) => d.fecha),
  });
});
...
router.post('/batches/:id/workers', async (req, res) => {
  ...
  for (const t of parsed.data.trabajadores) {
    let patient = null;
    if (t.nss) patient = await prisma.patient.findFirst({ where: { nss: t.nss } });
    if (!patient) {
      patient = await prisma.patient.findFirst({
        where: { fullName: { equals: t.nombre_completo.trim(), mode: 'insensitive' }, companyId: batch.companyId },
      });
    }
    const birth = t.fecha_nacimiento ? new Date(t.fecha_nacimiento + 'T00:00:00') : null;
    if (!patient) {
      patient = await prisma.patient.create({
        data: {
          fullName: t.nombre_completo.trim(),
          birthDate: birth && !isNaN(birth.getTime()) ? birth : null,
          nss: t.nss || null,
          phone: t.telefono || null,
          gender: t.genero || null,
          companyId: batch.companyId,
          medicalNotes: t.notas || null,
        },
      });
    }
